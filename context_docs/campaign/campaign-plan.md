...
**It is published to AWS Open Data, at `s3://tessera-embeddings/v1.1/dclimate.icechunk`.** Not to a
bucket of ours: AWS Open Data carries the storage cost of the finished dataset and serves it
publicly. Two things a reader should know about that address. **The suffix says what the format is**:
an Icechunk repository, opened with `icechunk.Repository.open` rather than `xr.open_zarr` — which is
why it is not named `.zarr` like the bucket's other resident, `v1.1/cambridge.zarr`, a genuine plain
Zarr v3 hierarchy. That also leaves `.zarr` free for the Zarr v3 translation Cambridge intends to
...
| `attempts_per_cell_in_cluster` | 2 | **The cheap retry.** One more go at a failed cell on the GPU fleet that is still standing — the cell re-enters the live work stream, reusing its kept mosaic and staged tiles, so it costs the tiles it actually lost. Covers "the work failed but the machine is fine"; a dead run is `max_dispatch_rounds`' job (§8). **Corrected in place:** until 2026-09-09 this retry ran after the stream had finished, through a per-cell path that called `run_inference` again — and a fleet's lifetime is exactly one such call, so every retried cell built a fleet from nothing on a cluster whose GPU nodes the autoscaler had already reclaimed. Measured on mellow-walrus: about 46 placed slots of 250 and ~230 tiles/hour against ~1,700, six to seven hours per cell |
...
| inside one **cluster** | `attempts_per_cell_in_cluster` (2, so one retry) | one cell, re-admitted to the live work stream on the GPU fleet that is still standing | a cell that failed while its machinery is fine and its inputs are still on disk |
...
| within **one round** | `immediate_refill` (off by default) | one settled cluster's still-missing cells, at once, in the slot it vacated | the WAIT itself — a roster that would otherwise sit behind the round's slowest sibling. It fixes latency, not a failure class: what it retries is what the next round would have retried anyway |
...
| **How much of the product the review has looked at** | reported, never graded: the review is a deliberate worst-first sample and a thousand cells were never going to be seen. It runs as a session rather than as a flow, so it is the one signal here that can reach zero without anything failing — which is why it is counted rather than assumed |
| **A batch of reviews that agrees unanimously** | the one reading of the review that IS graded. A reviewer collapsed onto a single answer looks exactly like a campaign going well, and in production there are no labels, so unanimity is the only symptom available. Re-score it against the calibration set before trusting the next batch |
