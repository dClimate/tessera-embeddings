...
Icechunk store.** Everything here is settled unless it appears in §10, which is the list of
...
**Grid.** Each zone group is its own UTM CRS on a fixed 10 m grid, snapped to the 2048-px
shard pitch — so a zone is about **933,888 × 67,584 px** (~63 Gpx per year, pole to pole and
...
join. Zones overlap slightly by construction, which is what makes that join possible.

**Time is exactly nine calendar years**, 2017–2025, one slot each. A `time` point is Jan 1 of
its year and `time_bnds` states the real half-open interval, because the slot holds a whole
Jan–Dec window rather than an instant. The axis is fixed at seeding: nothing appends years.

### The one thing every reader must do

`embeddings` is int8 and meaningless on its own. The real value is `embeddings * scales`,
broadcast over the band axis — one scale per **pixel**, taken as that pixel's largest absolute
value, so each pixel's largest dimension sits on the ±127 rail by construction.
...
`scales` is **NaN wherever no embedding exists** — ocean, a tile with no valid observation all
year, an unfilled year. That NaN is the authoritative "no data" mask: multiplying through it
...
**Absent shards are the normal case.** A zone is mostly ocean and nothing is written there;
...
level, and a consumer cannot tell ocean from a tile that yielded nothing — which is what the
provenance below is for.
...
and spatially clustered — byte-identical sets across independent fills of the same cell — and they
publish as fill, which is exactly what ocean reads as. So the store records them per year, and
**what ships with the data should say so**, alongside the deliberately optical-only regions of §10.
...
densest — which is also the slowest to ingest — idles a fleet about six hours per cluster-year
with finished mosaics already on disk. The ingest adapter's executor is sized to
...
That shape is doing three jobs at once, which is why it is a separate flow rather than a step:
...
2. **The error is DISTINGUISHABLE to monitoring, not merely present.** It raises its own exception
...
7. **Storage at petabyte scale prepared — see §7b.** Not a quota; a set of operational
...
**S2 duplicate copies step down, at every stage that can fail.** An unreadable preferred copy is
returned into the same ladder a write failure uses — including from the **coverage gate**, which is
a date's *first* compute and sits one stage before the ladder, so a single bad object cannot strand
a whole zone-year while a good older copy goes unused. Only an unreadable source converts that way;
anything else propagates. The processing-baseline map that sets the reflectance offset is derived
**per preparation, from that preparation's own items**, and travels back on the result — a map built
once per catalogue query, before duplicates are pruned, would describe a copy the loader never
opened and shift every pixel of a date silently.
...
**A cancellation is a request, not a fact.** Prefect marks a run Cancelling and a worker acts on
it later, so a child being replaced can keep writing across that gap. The flow follows a discarded
run to a *terminal* state and keeps its id registered until it gets there — dropping the id would
also remove the run from the shutdown sweep, in the same breath. If the run will not confirm within
five minutes, **the retry is refused rather than raced**. That costs a recoverable cell, and it is
...
> pinned into every fill, which is the only thing stopping one run straddling two images. Two
...
>   checked on the *append* path, deliberately, so a code change does not declare finished mosaics
...
> - **`ABSENT_MEANS_OFF` names the fields where having no opinion IS an opinion.** `allow_s2_only`
>   records "off" by being absent, for compatibility with stores predating the field — so manifest
>   validation skipping absent keys let a flag-ON run pass against a legacy store and mix both pixel
...
> **Reaching existing staging: what works and what only looks like it does.**
>
...
kill of a fill's flow body — every `finally` and context manager skipped — was recovered by the
terminal hook, which terminated all five GPU instances **at the same second**. That refutes the
...
> *when* it is safe to act, which is why the rule is here. Land such a change only after
...
| **A published cell with NO verdict** (§5) | nothing checked it. The dispatch is best-effort by design, so its absence is the record that one was lost; those cells need a validation run by hand |
