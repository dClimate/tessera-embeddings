...
The optical path took the same change one step further — the ROI mask's credentials are now
resolved per READ rather than frozen once per leg — and that is a throughput question, not a
correctness one. Measured 2026-08-05 on `global-tessera-dev` against an acceptance bar of **no
worse than 4-5%**. Folded in here from a separate note, because the measurement only means
anything beside the change it prices.
...
#### Method notes for this measurement
