...
| cell | `attempts_per_cell_in_cluster` (`sequential_fill` / `fill_zones_sequential`) | 2 | eligible. **Corrected in place:** this row used to describe eligibility as being by phase, decided in a pass that ran after the whole inference stream. Every phase is now eligible and the retry re-enters the LIVE stream — the cell goes to the back of the feeder's queue and is served by the fleet already standing. An `inputs/prepare` failure additionally gets `discard()` + `start()`, dispatched at once and non-blockingly, so the fresh ingest waits its turn behind the roster's others while inference and assembly carry on. Only an ASSEMBLY failure is still retried after the stream, and that path provisions nothing |
