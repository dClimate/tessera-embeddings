...
class AssemblyConfig:
    ...
    ``max_workers`` defaults to 16: a 16-worker pool was measured peaking near 40 GiB, which fits
    the 64 GiB inference flow-runner family — the smallest host any caller of this default runs on
    — at about two thirds of it rather than comfortably.

    **A bigger pool belongs to the runner that can hold it, not to this default.** A 32-worker pool
    was measured peaking near 94 GiB, which does NOT fit that 64 GiB family at all, and the
    campaign therefore passes
