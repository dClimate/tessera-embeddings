...
class AssemblyConfig:
    ...
    ``max_workers`` defaults to 16: each worker holds at most one staged-tile slice (~1-1.5 GB),
    so the pool peaks around ~24 GB — inside the 64 GiB inference flow-runner family, the smallest
    host any caller of this default runs on.

    **A bigger pool belongs to the runner that can hold it, not to this default.** 32 workers peak
    around ~48 GB, so the campaign passes ``n_assembly_workers=32`` in its chained-fill dispatch
    (``run_global_campaign.ASSEMBLY_WORKERS_ON_THE_LARGE_RUNNER``), which is the only deployment on
    the 244 GiB ``assembly_large`` family.

    Nothing here caps icechunk's request concurrency any more.
    ...
    #: for ("leaves headroom for n_workers=16 (~19 GiB)"). Safe on every host that takes this
    #: default; see the class docstring for the campaign's 32.
    max_workers: int = 16
