...
# the *resident input working set* is bounded separately via fixed-height
# northing strips (see actors._STRIP_HEIGHT), so a 2000x2000 chunk's peak host
# RAM is capped by the strip height rather than fixed by T x H x W. Independent
# of the storage chunk size written at ingest (config.ingest.INGEST_CHUNK_SIZE).
