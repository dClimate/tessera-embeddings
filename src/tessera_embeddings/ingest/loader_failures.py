...
Both are known only on the reading process, and neither reaches the caller on its own — one is
...
``odc.stac.load`` names the object it gave up on in its OWN log record and raises an exception
that does not carry it, so a caller knows *a* source was unreadable but not WHICH — and the name
sits in one log stream among hundreds in a fleet. Everything downstream then guesses: the
duplicate ladder steps every duplicated tile in the date down one copy, so a date carrying
hundreds of duplicated tiles swaps hundreds of copies to recover from one bad object, and a date
whose bad object has no alternate still walks the whole ladder before giving up.

A logging handler on each reader process records the hrefs the loader aborted on; the caller
collects them after a failure and maps them back to the tile-dates that produced them.

**Attribution is best effort, and no recovery may depend on it.** A worker that died with the
read, a cluster already gone when the caller asks, or a loader that words the message differently
all yield nothing. An empty answer therefore means "attribute nothing" and leaves the caller on
its unattributed path — never "nothing was at fault".

Keeping the reason
------------------

A read failure's reason is its CAUSE: GDAL states it (a codec, a status, a refusal) and rasterio
wraps that in a generic ``Read failed. See previous exception for details.``, so every verdict
this pipeline reaches is a verdict about the chain, never about the exception it arrives as.
...
publish ``args`` as a read-only property, so the assignment throws and the whole chain fails to
unpickle. Dask notices before sending and substitutes a plain ``Exception`` holding the outer
exception's repr: one line of text, ``__cause__`` of ``None``, reason gone. The undecidable
failure was the architecture discarding the classifier's only evidence, not a gap in the
classifier. :func:`keep_causes_picklable` removes the assignment tblib cannot make, giving those
classes a ``__reduce__`` that rebuilds them from their own ``args`` through their own constructor.

Keeping the reason GDAL did not raise
-------------------------------------

An intact chain is still only what the reader chose to RAISE, and GDAL does not raise everything
it knows. A refused object comes back as an error document where the imagery should be; GDAL
states the refusal in its own log and hands the document to the codec, which raises the only
thing it can see — a decode failure. So the chain says ``ZIPDecode`` and the classifier says
these bytes are bad, while the words saying the service refused sit one log line away.

The verdicts are opposites: unreadable means give the date up, refused means wait and give up
nothing — and a date given up below a store's append-only maximum cannot be back-filled by the
re-run meant to recover it. A whole outage's worth of recoverable dates can be spent on which of
two errors GDAL happened to raise last.
...
GDAL's own fetch threads report to GDAL's process-wide handler, which writes to stderr and to
nothing a ``logging.Handler`` can reach. :func:`hear_gdal_from_every_thread` gives that handler
somewhere to forward to, in rasterio's own wording.

**Only refusals are recorded, and that is what makes the direction safe.** A line is kept only if
the classifier reads THAT LINE ALONE as ``PROVIDER_REFUSED`` or ``OUR_CREDENTIAL``. Refusal is
tested before anything about the bytes, so attaching such a line can only move a verdict INTO
those two — never into "unreadable" or "absent", and so never into giving a date up. The cost is
patience: a date that borrows another's refusal waits out the write budget, fails its leg with
the axis unmoved, and is judged again alone on the re-dispatch.
"""
...
def _strip_band_suffix(href: str) -> str:
    ...
    The message reads ``...B02.tif:1``. The trailing index is the band within the file, not part
    of the object's identity, and leaving it on defeats comparison against a catalogue asset href.
    """
...
def href_key(href: str) -> str:
    ...
    ``https://bucket.s3.<region>.amazonaws.com/key``, and a path-style URL puts the bucket in the
    path. The last two path segments — granule directory and file — are the same in all three and
    unique within a collection, so they are the key.
    """
...
class _AbortedReadHandler(logging.Handler):
    ...
    A handler rather than a wrapper around the load call: the loader reports the object from
    inside its own thread pool, so nothing at the call site sees it.
    """
...
class _LoggedRefusalHandler(logging.Handler):
    ...
    Filtered by the CLASSIFIER rather than a marker list of its own, so there is one vocabulary
    for what a refusal is. A line is kept only if it reaches :data:`_KEEPABLE` read alone — which
    also settles corroboration, since GDAL's wording carries its own ``CPLE_`` class name and
    GDAL reads nothing here but source imagery.

    Everything else GDAL says is dropped, and that is a safety property: a benign 404 probing for
    a sidecar, kept, would be evidence that an object is ABSENT.
    """
...
def hear_gdal_from_every_thread() -> None:
    ...
    **per thread**. GDAL's ranged reader fetches on threads of its own, which never enter a
    ...
    to stderr and to no logger at all. :class:`_LoggedRefusalHandler` is a ``logging.Handler`` and
    ...
    **In rasterio's own wording, from rasterio's own** ``code_map``. The classifier corroborates
    that a line is the source READER's from GDAL's vocabulary, so ``CPLE_AppDefined in <message>``
    ...
    **Not levelled the way rasterio levels it, deliberately.** rasterio downgrades ``CE_Failure``
    to ``INFO`` because one of its calls may emit several and still succeed. A message reaching
    the process-wide handler has no such call to judge it, and ``INFO`` is below both the logger's
    ...
    **Installed once, under a lock.** GDAL keeps the function POINTER and only the module-level
    reference keeps the callback object alive; two installs would leave the first collectable
    while GDAL and the chained handler still hold its address, so the next GDAL error would call
    into freed memory. Check and install must be one critical section.

    Never raises. A process that cannot install this reads exactly as it did before.
    """
    with _install_lock:
        if _forwarders:
            return
...
def install_capture() -> None:
    ...
    A handler sees a record only if its logger is enabled for that level, and GDAL's refusals
    arrive as warnings. Nothing in this package raises a logger above ``WARNING``, and lowering
    GDAL's own would emit its whole warning stream to the root handlers, so the level is read
    rather than set.

    The forwarder goes in alongside because it is the same sensor: these handlers hear what GDAL
    says to a logger, and it is what makes GDAL say the rest of it to one.
    """
...
def _drain(buffer: deque[str]) -> list[str]:
    ...
    Draining rather than reading, so one failure's evidence cannot be inherited by the next: a
    date that fails, steps down and fails again must be judged on the second attempt's objects.
    """
    with _lock:
        found = list(buffer)
        buffer.clear()
...
def read_local_refusals() -> list[tuple[float, str]]:
    ...
    Reading rather than draining is the safety property. Two reads are in flight whenever the
    optical path pipelines a date: the look-ahead prepares date N+1, whose coverage gate reads,
    while date N's write is still reading. Both enter their own ``read_failure_context``, and a
    destructive collection let whichever failed FIRST take the other's evidence — the second
    failure then saw only the codec's complaint, classified it as unreadable data, and gave its
    date up.

    **Ages, not timestamps, and the decision is not taken here.** This runs on a worker and the
    caller runs elsewhere, so their clocks are not comparable but an elapsed time is. Whether a
    ...
    trip took longer than the read, classifying a refusal as unreadable data and costing a date.

    Separate from the href buffer, which is still drained destructively: one buffer would mean
    the caller that classifies destroys the evidence the optical copy ladder attributes from.
    """
    now = time.monotonic()
    with _lock:
...
def _reduce_by_args(exc: BaseException) -> tuple[type[BaseException], tuple[Any, ...]]:
    ...
    Faithful rather than merely sufficient: GDAL's constructor takes exactly the triple its
    ``args`` reports, so the rebuilt exception carries the error class, the CPL error number and
    the message the codec wrote, not a flattened string.
    """
...
def rescues_are_installed() -> bool:
    ...
    The reducer only. Both captures matter to a verdict — the href sharpens attribution, the
    refusal decides whether a refusal GDAL only logged is reachable at all — but neither is gated
    here: a worker missing them costs dates in an outage, while a worker missing the reducer
    cannot classify anything and strands a zone-year. Only the second is worth refusing a leg
    over, and installation is one call, so in practice they travel together.
    """
...
class AbortedReadCapture(WorkerPlugin):
    ...
    A plugin rather than ``client.run``, for the same reason the credential broadcast is one:
    workers that join later — under adaptive scaling, most of them — would otherwise read without
    capturing, and those are exactly the reads a wide fleet fails.
    """
...
def install_capture_everywhere(client: dask.distributed.Client | None) -> None:
    ...
    undecidable read failure strands a whole zone-year, so a fleet that cannot classify refuses
    to start. Verified rather than assumed, because registration returning is not the worker
    having run ``setup``, and the ask doubles as the barrier against reading before it has.
    """
    install_capture()
    keep_causes_picklable()
    if client is None:
...
def _collect[T](client: dask.distributed.Client | None, read: Callable[[], list[T]], what: str) -> list[T]:
    ...
    error with an unrecoverable one. A worker that could not answer is COUNTED rather than
    dropped — see the call below.
    """
    found = read()
    if client is None:
        return found
...
def collect_logged_refusals(client: dask.distributed.Client | None, since: float | None = None) -> list[str]:
    ...
    agreeing or on the round trip being quick.

    **The cutoff has to be taken after the call, not before it.** Measured before, it excluded
    the scheduling and RPC latency that the workers' ages already include, so a line logged
    during a short read looked older than the read itself and was thrown away — leaving the codec
    exception classified as unreadable data, which costs the date. The residual error is the
    return leg alone, and it biases towards KEEPING a line, which costs a wait rather than a date.
    """
...
def carry_logged_refusal(exc: BaseException, client: dask.distributed.Client | None, since: float | None = None) -> int:
    ...
    attaching is what makes one classifier decide from the whole of what is known.
    ...
    **Attached, not merely read.** A caller that classified and discarded would leave the next
    ...
    ``since`` is when the read being judged began, by this process's ``time.monotonic``; what it
    buys and costs is on :func:`read_local_refusals`.

    The evidence is cluster-wide, so a date failing while ANOTHER date's read is being refused
    can take the other's lines into its note — the same race the href attribution runs, bounded
...
def refusal_wait_out(client: dask.distributed.Client | None) -> Callable[[BaseException], bool]:
    ...
    gathering is what arms it: a predicate reading only the exception cannot see the refusal,
    declines, and spends the ordinary three attempts on an outage the budget exists to outlast.
    ...
    **The window is the WRITE, not the attempt, and that is the load-bearing part.** The predicate
    ...
    and an outage states its refusal while it is refusing rather than on a schedule of ours.
    Re-armed per attempt, the question silently became "was anything refused in the last few
    seconds", which a recovering outage answers correctly with NO one attempt before the write
    would have succeeded — withdrawing patience exactly when it was about to pay off.

    One window also makes the two readings of one failure AGREE. ``read_failure_context`` judges
    the same failure over the whole write when the retry finally gives up, so a narrower window
    here let the same words be a refusal to one reader and not the other — the enclosing context
    raising ``ProviderRefusedReadsError`` over a failure this predicate had just declined to wait
    for.

    Nothing here remembers a past verdict: the window is derived, so the evidence is re-read and
    re-classified on every attempt, and a write whose window holds no refusal never waits. A write
    that keeps seeing one is bounded by the caller's budget, unchanged —
    :data:`~tessera_embeddings.storage.zarr_store.WAIT_OUT_BACKOFF_S` of accumulated backoff,
    after which the write fails with its date unskipped and its time axis unmoved.
    """
...
def label_objects(hrefs: Iterable[str]) -> str:
    ...
    Granule directory and file only: the bucket and collection prefix are the same for every
    object in a run, so including them makes each entry four times longer for no distinction.
    """
...
def implicated_tile_dates(
    ...
    An href that matches nothing is dropped silently: it belongs to another date — plausible,
    since a fleet's workers read many — and attributing it here would step down a tile that read
    perfectly well.
    """
...
def implicated_items(items: Iterable[Any], hrefs: Iterable[str]) -> list[Any]:
    ...
    :func:`implicated_tile_dates` answers "which tile-dates failed", the granularity a caller
    reports at. This answers "which copies failed", the granularity a fallback should STEP at: a
    tile-date can hold several distinct acquisitions, and stepping the one whose alternate ranks
    highest downgrades a healthy acquisition to an older baseline while leaving the unreadable
    one selected.

    Same two-way matching and the same silent drop of an unmatched href — see
    :func:`implicated_tile_dates`.
    """
