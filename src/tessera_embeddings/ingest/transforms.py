...
def amplitude_to_db(ds: xr.Dataset) -> xr.Dataset:
    """Convert OPERA RTC-S1 linear amplitude to scaled dB as int16.

    Ported from tessera_preprocessing/s1_fast_processor.py:758-797.
    Formula: (20 * log10(amplitude) + 50) * 200, clipped to [0, 32767] as int16.

    Applied per data variable in the dataset. This is a lazy Dask
    operation — no data is materialized.

    Args:
        ds: xarray Dataset with float32 amplitude variables

    Returns:
        Dataset with int16 scaled dB variables
    """
    result = {}
