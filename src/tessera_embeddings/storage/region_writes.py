"""Region-write geometry: chunk alignment and padding.

Pure data-transform helpers backing the region-overwrite path in
...
merge, commit) lives next to the public ``write_region`` wrapper in
``zarr_store``; this module is the geometry that wrapper leans on.
"""
