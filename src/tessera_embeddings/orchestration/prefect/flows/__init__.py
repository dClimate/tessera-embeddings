...
Each flow file binds a piece of domain logic to a substrate. The shape
...
3. Inside the context, call an inner ``@flow`` with
   ``.with_options(task_runner=...)`` so the task runner binds late to
   the cluster's scheduler address. This is a Prefect idiom — do not
   try to collapse it into a single flow.
