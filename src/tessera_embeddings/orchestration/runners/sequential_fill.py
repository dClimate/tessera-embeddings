...
#: The most wall-clock ONE trailing assembly may take before the drain stops waiting for it.
...
class TrailingAssemblyWedgedError(RuntimeError):
    """The drain gave up on an assembly that never returned, and the thread running it is still alive.
...
def drain_trailing_assemblies(
    ...
    *,
    ...
    ``finalizer.shutdown(wait=True)`` was the drain until 2026-09-04, and it has no bound: one
    ...
    Returns:
        The number of assemblies abandoned — ``0`` on a clean drain.
    """
...
def fill_zones_sequential(
    *,
    ...
    assembly_pending = 0
    #: Every future the finalizer has been handed, so the drain can wait on completions with a
