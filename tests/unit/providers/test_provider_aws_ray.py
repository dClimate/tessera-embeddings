...
class TestGpuWorkerLadderParsing:
    ...
    def test_parses_pairs_in_order_tolerating_whitespace_and_trailing_comma(self) -> None:
        assert _parse_gpu_worker_ladder(" g6e.xlarge:16 , g5.2xlarge:4 ,") == [
            ("g6e.xlarge", 16),
            ("g5.2xlarge", 4),
        ]
...
class TestGpuWorkerLadderApplication:
    ...
    def test_applies_counts_to_the_named_rungs(self) -> None:
        cfg = yaml.safe_load(DEFAULT_CLUSTER_TEMPLATE.read_text())
        _apply_gpu_worker_ladder(cfg, "g6e.xlarge:16,g5.2xlarge:4")
        got = {
            name: nt["max_workers"]
        ...
        assert by_type["g5.2xlarge"] == 4
        assert all(v == 0 for t, v in by_type.items() if t not in ("g6e.xlarge", "g5.2xlarge")), by_type
    ...
    def test_is_authoritative_not_additive(self) -> None:
        ...
        _apply_gpu_worker_ladder(cfg, "g5.2xlarge:4")
    ...
    def test_raises_the_cluster_ceiling_to_fit_the_ladder(self) -> None:
        ...
        _apply_gpu_worker_ladder(cfg, "g6e.xlarge:400,g5.2xlarge:400")
...
def test_resolve_ray_config_reads_the_ladder_from_ssm() -> None:
    ...
    boto3.client("ssm", region_name=REGION).put_parameter(
        ...
        Value="g6e.xlarge:16,g5.2xlarge:4",
        Type="String",
    )
    resolved = _resolve_ray_config(
        DEFAULT_CLUSTER_TEMPLATE, region=REGION, ami_ssm_name=ami_param, ssm_prefix=SSM_PREFIX
...
class TestRayNodeTypePreference:
    ...
    def test_the_two_single_gpu_rungs_are_ranked_by_gpu_density_not_size(self) -> None:
        """`g5.2xlarge` scores BELOW `g6e.xlarge`, so the fallback never displaces production.
        ...
        """
        node_types = _autodetected_node_types("g6e.xlarge:500,g5.2xlarge:500")
        scorer = _scorer()
        demand = [ACTOR_BUNDLE] * 250
        xl = scorer(node_types["gpu-workers-ondemand"]["resources"], demand, "x")
        xl2 = scorer(node_types["gpu-workers-ondemand-a10g-2xl"]["resources"], demand, "y")
        assert xl > xl2
    ...
    def test_max_workers_zero_makes_a_rung_unreachable(self) -> None:
        ...
        chosen, residual = get_nodes_for(
            _autodetected_node_types("g5.2xlarge:2"), {}, "head", 600, [ACTOR_BUNDLE] * 250, _scorer()
        )
        assert dict(chosen) == {"gpu-workers-ondemand-a10g-2xl": 2}
        # 2 nodes x 1 GPU place 2 of the 250; the rest stay unplaced rather than
        # landing on the rungs the ladder closed — including the production rung,
        # which the ladder closed by not naming it.
        assert len(residual) == 250 - 2
    ...
    def test_declared_resources_match_the_ec2_catalogue(self) -> None:
        ...
        for name, cfg in node_types.items():
            ...
            checked += 1
        assert checked == 5, "every node type in the template must declare resources"
...
class TestL4Rungs:
    ...
    def test_a_ladder_can_name_both_fallback_cards(self) -> None:
        """Both fallback rungs are reachable through the same ladder key, without a
        release — which is the point of shipping them inert rather than not at all.
        """
        node_types = _autodetected_node_types("g6.2xlarge:3,g5.2xlarge:3")
        opened = {name: cfg["max_workers"] for name, cfg in node_types.items() if cfg["max_workers"]}
        assert opened == {"gpu-workers-ondemand-l4-2xl": 3, "gpu-workers-ondemand-a10g-2xl": 3}
