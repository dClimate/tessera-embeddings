...
# ---------------------------------------------------------------------------
# Transient object-store refusals on a block write.
# ---------------------------------------------------------------------------


class _FlakyArray:
    """A zarr-array stand-in that refuses the first ``fail_times`` assignments."""

    def __init__(self, fail_times: int, message: str) -> None:
        self.fail_times, self.message = fail_times, message
        self.assignments = 0

    def __setitem__(self, _selection, _value) -> None:
        self.assignments += 1
        if self.assignments <= self.fail_times:
            raise RuntimeError(self.message)


class TestTransientWriteRetry:
    """Why this exists: staged READS during assembly carry botocore
    ``max_attempts: 10, mode: adaptive``, but writes go through icechunk's Rust S3 client,
    which exposes no retry knob — so a single transient refusal killed a whole assembly.

    Measured 2026-08-28: 22 `SignatureDoesNotMatch` refusals on the assembly path cost FOUR
    cells, while 494 of the same refusal on the ingest path cost none, because ingest writes
    sit under Prefect task retries and these did not.
    """

    SEL = (slice(0, 1), slice(0, 8), slice(0, 8))

    def test_a_signature_refusal_is_retried_and_succeeds(self, monkeypatch) -> None:
        """The refusal that actually cost us cells. `SignatureDoesNotMatch` means the secret
        did not match the access key, which for a role credential is a refresh race.
        """
        monkeypatch.setattr(shard_writer.time, "sleep", lambda _s: None)
        arr = _FlakyArray(2, 'Error { code: "SignatureDoesNotMatch", message: "..." }')
        shard_writer._assign_block_with_retry(arr, self.SEL, object(), worker_index=3, group="35N")
        assert arr.assignments == 3, "two refusals then success"
    ...
    def test_every_transient_code_is_retried(self, monkeypatch, code: str) -> None:
        monkeypatch.setattr(shard_writer.time, "sleep", lambda _s: None)
        arr = _FlakyArray(1, f"unhandled error ({code})")
        shard_writer._assign_block_with_retry(arr, self.SEL, object(), worker_index=0, group="47N")
        assert arr.assignments == 2

    @pytest.mark.parametrize("code", ["AccessDenied", "InvalidAccessKeyId", "NoSuchBucket"])
    def test_a_permanent_refusal_is_not_retried(self, monkeypatch, code: str) -> None:
        """Retrying a permission or naming error buys nothing and delays the diagnosis."""
        monkeypatch.setattr(shard_writer.time, "sleep", lambda _s: None)
        arr = _FlakyArray(1, f"unhandled error ({code})")
        with pytest.raises(RuntimeError, match=code):
            shard_writer._assign_block_with_retry(arr, self.SEL, object(), worker_index=0, group="47N")
        assert arr.assignments == 1, "refused once, not retried"

    def test_it_gives_up_rather_than_hanging_on_a_broken_credential(self, monkeypatch) -> None:
        """Bounded, so a genuinely wrong secret fails the assembly promptly instead of
        retrying a doomed write for the length of the run.
        """
        monkeypatch.setattr(shard_writer.time, "sleep", lambda _s: None)
        arr = _FlakyArray(99, "unhandled error (SignatureDoesNotMatch)")
        with pytest.raises(RuntimeError, match="SignatureDoesNotMatch"):
            shard_writer._assign_block_with_retry(arr, self.SEL, object(), worker_index=0, group="35N")
        assert arr.assignments == shard_writer._WRITE_ATTEMPTS

    def test_no_bare_status_numbers_in_the_retryable_set(self) -> None:
        """A bare "500" or "416" also matches the hex inside an ECS task ARN, which has
        produced phantom findings in this repo before. Named codes only.
        """
        assert not [c for c in shard_writer._RETRYABLE_STORE_CODES if c.isdigit()]

    def test_the_backoff_is_bounded_and_short(self) -> None:
        """At most a few seconds of sleep per block — negligible against a shard's own
        read and encode, which is what makes retrying cheaper than losing the assembly.
        """
        total = sum(shard_writer._WRITE_BACKOFF_BASE_S * 2**i for i in range(shard_writer._WRITE_ATTEMPTS - 1))
        assert total < 10.0
