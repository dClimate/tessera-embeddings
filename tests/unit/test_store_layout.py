...
from tessera_embeddings.config.store_layout import GLOBAL_V1, LEGACY, OBS_COUNT_VARS


def test_global_v1_embeddings_geometry():
    k = GLOBAL_V1.for_var("embeddings").create_kwargs((9, 4096, 4096, EMBEDDING_DIM))
...
def test_global_v1_scales_is_sharded_pcodec():
    k = GLOBAL_V1.for_var("scales").create_kwargs((9, 4096, 4096))
...
def test_legacy_embeddings_unsharded():
    k = LEGACY.for_var("embeddings").create_kwargs((1, 500, 500, EMBEDDING_DIM))
...
def test_obs_vars_resolve_and_unknown_raises():
    for var in OBS_COUNT_VARS:
        assert GLOBAL_V1.for_var(var).dtype == "uint16"
    with pytest.raises(KeyError):
        GLOBAL_V1.for_var("nonsense")


def test_chunks_and_shards_clamp_to_small_shape():
    # A store smaller than one nominal shard must still create cleanly.
    k = GLOBAL_V1.for_var("embeddings").create_kwargs((1, 300, 300, EMBEDDING_DIM))
    assert k["chunks"] == (1, 256, 256, EMBEDDING_DIM)
...
def test_sharded_roundtrip_and_fill(tmp_path):
    ...
    emb_kwargs = GLOBAL_V1.for_var("embeddings").create_kwargs((1, 2048, 2048, EMBEDDING_DIM))
    emb = root.create_array("embeddings", **emb_kwargs)
    scl = root.create_array("scales", **GLOBAL_V1.for_var("scales").create_kwargs((1, 2048, 2048)))
