...
def test_the_default_pool_is_sixteen_and_the_campaign_asks_for_thirty_two() -> None:
    """The worker count belongs to the runner that can hold it. Measured per task, 16 workers peak
    near 40 GiB and fit the 64 GiB host every default-taking caller runs on; 32 peak near 94 GiB,
    which does NOT fit it, and only the chained campaign fill is on the 244 GiB family, so only it
    asks explicitly.
    """
