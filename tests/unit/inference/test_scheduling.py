...
class TestPollTracker:
    ...
    @pytest.mark.parametrize(
        ("cells", "expected"),
        [
            ({"15N-2019"}, "Progress [15N-2019]:"),
            # A chained session overlaps one zone's tail with the next zone's head.
            ({"16N-2019", "15N-2019"}, "Progress [15N-2019, 16N-2019]:"),
            # A single-area run's id is a content digest: it names nothing, so nothing is shown.
            (set(), "Progress: 0/10 chunks done"),
        ],
        ids=["one-cell", "chained-boundary", "unnamed"],
    )
    def test_the_line_names_the_cells_with_work_in_flight(self, caplog, cells, expected) -> None:
        with caplog.at_level(logging.INFO, logger="test"):
            _poll({"c_0": (3, 10, 10.0, "inference")}, stall_threshold=300.0, max_stalls=10, cells=cells)
