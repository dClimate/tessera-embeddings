(tessera-embeddings) [ssm-user@ip-10-200-102-141 scripts]$ uv run python -m scale_tests.report   --run-id run1
Wrote /home/ssm-user/tessera-embeddings/scripts/scale_test_results/run1/report.md (6749 chars)
...
| D2 chunk shape | T1 point p95 by variant | c256_full=193.272ms | pick best weighted variant |
| D3 sharding | sharded vs full point p95 | no data (need both variants) | adopt only if reads win |
